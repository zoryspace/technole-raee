...
collectionsRouter.get("/", auth, async (_req, res, next) => {
  try {
    const { rows: cols } = await pool.query(
      "SELECT * FROM collections ORDER BY date DESC"
    );
...
collectionsRouter.post("/", auth, async (req, res, next) => {
...
collectionsRouter.put("/:id", auth, async (req, res, next) => {
  const { clientId, date, address, technician, serviceType } = req.body;
  try {
    const { rows } = await pool.query(
...
collectionsRouter.put("/:id/devices", auth, async (req, res, next) => {
...
collectionsRouter.delete("/:id", auth, async (req, res, next) => {
  try {
